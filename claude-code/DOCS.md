...
| `ha-usage [days]` | Summarize Claude token usage (today / N days / all-time, per model) and prompt-API cost. Console usage comes from Claude's session history; chat requests from the prompt API's lines in the audit log. `--json` also says whether the console usage could be read (`available`, and `error` when reading failed) |
